...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        h1_relu, cache_relu = ReLU.forward(h1)
        scores, cache2 = Linear.forward(h1_relu, W2, b2)
        ...
        dh1_relu, dw2, db2 = Linear.backward(dscores, cache2)
        dh1 = ReLU.backward(dh1_relu, cache_relu)
        dx, dw1, db1 = Linear.backward(dh1, cache1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_last = Linear.forward(h, W_last, b_last) 
        ...
        for i in range(self.num_layers-1, 0, -1):
            cache_new = caches[i - 1]
            if self.use_dropout:
                cache, dropout_cache = cache_new
                dh = Dropout.backward(dh, dropout_cache)
            else:
                cache = cache_new    
            dh, dW, db = Linear_ReLU.backward(dh, cache)
...
def create_solver_instance(data_dict, dtype, device):
    model = TwoLayerNet(hidden_dim=256, reg=1e-4, weight_scale=1e-3, dtype=dtype, device=device)
    #############################################################
